<template>
  <el-card>
    ...
    <el-button class="my-3" :disabled="!fileList.length" @click="handleUpload"
      >上传({{ fileList.length }}/{{ config.limit }})</el-button
    >
    ...
    <el-table :data="filesData" border>
      ...
      <el-table-column prop="url" label="点击查看">
        <template #default="{ row: { url, originalname } }"
          ><a :href="url" target="_blank">{{ originalname }}</a></template
        >
      </el-table-column>
